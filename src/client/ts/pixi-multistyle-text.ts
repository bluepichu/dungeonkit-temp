...
// Adapted from Pixi Multistyle Text
...
export default class MultiStyleText extends Text {
	...
	public set style(styles: TextStyleSet) {
		...
		for (let style in styles) {
			if (typeof styles[style].dropShadowColor === "number") {
				styles[style].dropShadowColor = utils.hex2string(styles[style].dropShadowColor as number);
			}

			if (typeof styles[style].fill === "number") {
				styles[style].fill = utils.hex2string(styles[style].fill as number);
			}

			if (typeof styles[style].stroke === "number") {
				styles[style].stroke = utils.hex2string(styles[style].stroke as number);
			}

			if (style === "def") {
				Object.assign(this.textStyles["def"], styles[style]);
			} else {
		...
		this.dirty = true;
		this._style = this.textStyles["def"];
	}
	...
	private _getTextDataPerLine (lines: string[]) {
		...
		let styleStack = [Object.assign({}, this.textStyles["def"])];
	...
	public updateText(): void {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			for (let j = 0; j < outputTextData[i].length; j++) {
				...
				this.context.font = Text.getFontStyle(outputTextData[i][j].style);
				...
				outputTextData[i][j].fontProperties = Text.calculateFontProperties(this.context.font);
		...
		for (let i = 0; i < outputTextData.length; i++) {
			...
			for (let j = 0; j < line.length; j++) {
				...
				this.context.font = Text.getFontStyle(textStyle);
