...
function numberToTile(val: number): Game.Crawl.DungeonTile {
	switch (val) {
		case 0:
			return { type: "wall", roomId: -1, stairs: false };
		case 10:
			return { type: "wall", roomId: -1, stairs: false };
		case 9:
			return { type: "open", roomId: 0, stairs: false };
		default:
			if (val > 0) {
				return { type: "wall", roomId: 0, stairs: false };
			} else {
				return { type: "open", roomId: -(val + 1), stairs: false };
			}
	}
}
